...
**Tool discovery**: telegram-me tools appear as **deferred tools** (listed in `<available-deferred-tools>`). Before concluding they are unavailable, use `ToolSearch` with query `"telegram-me"` to fetch their schemas. Only if `ToolSearch` returns no telegram-me tools should you report them as unavailable.

### Auto-start

1. Fetch telegram-me tool schemas: `ToolSearch` query `"telegram-me"` — if no tools found, report unavailable and stop
2. Create flag: `touch /tmp/telegram-me-autonomous.flag`
3. Check for `~/.telegram-me/resume/{project}.json` — if found, read saved context, delete file, notify "Resuming from rate-limited session", ask "Continue with this task?" with `["Yes, continue", "New task"]`
4. Call `get_user_message` for buffered messages
5. Notify: "Autonomous mode active."
6. Enter autonomous loop
...
| `/fast` | Apply the provider fast preset for the current project |
